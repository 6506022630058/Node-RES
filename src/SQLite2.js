...
app.get('/summary', (req, res) => {
    db1.all('SELECT students.name AS student, courses.name AS course FROM students JOIN std_crss ON students.id = std_crss.std_id JOIN courses ON courses.id = std_crss.crs_id; ', (err, rows) => {
        if (err) {
            res.status(500).send(err);
        }   else {
            res.json(rows);
